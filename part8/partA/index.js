...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      let retval = books
      if (args.author) {
        retval = retval.filter(b => b.author === args.author)
      }
      if (args.genre) {
        retval = retval.filter(b => b.genres.includes(args.genre))
      }
      return retval
    }
  },
  ...
  Mutation: {
    addBook: (root, args) => {
      ...
      if (!authorExists) {
        authors = authors.concat({ "name": args.author, "born": null, "bookCount": 1 })
      }
      return args
    },
